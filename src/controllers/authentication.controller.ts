...
async function login(request: Request, response: Response) {
    try {
        ...
        response.status(200).json({ accessToken: access_token, refreshToken: refresh_token, token_type: 'Bearer', expires_in: 900 });
...
function generateTokens(username: string) {
    const access_token = sign({ username, type: TokenType.ACCESS }, process.env.ACCESS_SECRET as string, { expiresIn: "15m" });
    const refresh_token = sign({ username, type: TokenType.REFRESH }, process.env.REFRESH_SECRET as string, { expiresIn: "7d" });
    return { access_token, refresh_token };
}

function extractAndDecodeBasicCredentials(authHeader: string): { username: string, password: string } {
